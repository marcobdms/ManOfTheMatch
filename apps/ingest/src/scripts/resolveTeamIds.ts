...
// `teams.source_ids`. Run manually, once, after apps/ingest is deployed with
...
if (missing.length) {
  console.error(`[resolveTeamIds] faltan ${missing.join(', ')} — copia apps/ingest/.env.example a .env`);
  process.exit(1);
}

const { COMPETITIONS } = await import('@motm/shared');
